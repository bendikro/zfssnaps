...
from util import get_filesystem_match, get_snapshot_match, delete_snapshots, do_snapshots, list_snapshots
...
def main():
    ...
    argparser.add_argument("--no-date", help="Do not prepend date to name of new snapshots.", required=False, action='store_true')

    argparser.add_argument("-l", "--list", help="List snapshots.", required=False, action='store_true')
    argparser.add_argument("-R", "--recursive", help="Match recursively on filesystems.", required=False, action='store_true')
    argparser.add_argument("-s", "--simulate", help="Print the commands that would be run without executing.", required=False, action='store_true')
    argparser.add_argument("-v", "--verbose", help="Print the commands.", required=False, action='store_true', default=True)
    argparser.add_argument("-f", "--file-system",  help="File systems to operate on. May be given multiple times", required=False, action='append')
    ...
    #group.add_argument("-r", "--rollback", nargs=1, help="Rollback a snapshot.", required=False)
